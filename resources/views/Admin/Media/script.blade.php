...
<script type="text/javascript">
    ...
    $('#formSubmit').submit(function (e) {
        e.preventDefault();
        saveOrUpdate( save_method == 'add' ?"{{ route('Media.create') }}" : "{{ route('Media.update') }}");
    });
    ...
    function deleteFunction(id,type) {
        if (type == 2 && checkArray.length == 0) {
            ...
            url= "/Admin/Media/destroy/" + checkArray + '?type=2';
            deleteProccess(url);
            checkArray=[];
        }
    }
...
<script>
    function ChangeStatus(status,id) {
        ...
        $.ajax({
            url : '/Admin/Media/ChangeStatus/' +id +'?status='+status,
            type : 'get',
            success : function(data){
                $.toast().reset('all');
                table.ajax.reload();
