...
class MediaController extends Controller
{
    ...
    public function save_Media($request,$Media){
        ...
        if($request->image) {
            deleteFile('Media',$Media->image);
            $Media->image=saveImage('Media',$request->image);
        }
        $Media->save();
    }
    ...
    private function mainFunction($data)
    {
        return Datatables::of($data)->addColumn('action', function ($data) {
            ...
            $image = '<a href="'. getImageUrl('Media',$data->image).'" target="_blank">'
                .'<img  src="'. getImageUrl('Media',$data->image) . '" width="50px" height="50px"></a>';
            return $image;
